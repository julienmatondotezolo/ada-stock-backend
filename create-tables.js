...
const connectionString = `postgresql://postgres:[YOUR-PASSWORD]@db.${projectId}.supabase.co:5432/postgres`;
...
async function createTables() {
  try {
    ...
    // Check categories
    const categories = await client.query('SELECT name, name_nl FROM stock_categories ORDER BY sort_order');
    console.log(`📂 Inserted ${categories.rows.length} categories:`);
    categories.rows.forEach(cat => {
      console.log(`  • ${cat.name} (${cat.name_nl})`);
    });
